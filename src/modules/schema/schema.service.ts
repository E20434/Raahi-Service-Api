...
export class SchemaService {
  ...
  // async createServiceConfig(dto: CreateServiceConfigDto) {
  //   const { meta, special_elements = [], asset_types = [] } = dto;
    
  //   // Resolve serviceId by service_key (meta.service_type)
  //   const service = await this.serviceRepo.findOne({
  //     where: { serviceKey: meta.service_type }
  //   });
    
  //   if (!service) {
  //     throw new NotFoundException(`Service with type (key) '${meta.service_type}' not found`);
  //   }
    
  //   // Auto-generate schema_key
  //   const schemaKey = `${meta.service_type}_v${meta.schema_version}`;
  ...
  //         displayOrder: index, // Defaulting display_order to the array index
  //       });
  //     });
  ...
  async createServiceConfigById(dto: CreateServiceConfigByIdDto) {
    const { service_id, meta, special_elements = [], asset_types = [] } = dto;
